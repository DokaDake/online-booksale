...
import com.laver.mbook.util.MD5Utils;
...
import com.laver.mbook.service.IUserService;
...
public class UserController {
	...
	public boolean checkPhone(String phone){
		boolean succ = userService.checkPhoneIsRegist(phone);
		return succ;
	}
	...
	public boolean createCode(HttpSession session,String phone){
//		int code = VerficationCode.createCode(phone);
...
//		}
	...
	public String login(HttpSession session,String phone, String password) {
		if (!StringUtils.isEmpty(phone) && !StringUtils.isEmpty(password)) {
			String newCode= phone+"#"+password;
			Object code = session.getAttribute("code");
			String oldCode = (String) code;
			if (newCode.equals(oldCode)){
				//每次登入成功后将验证码失效
				session.removeAttribute("code");
				session.setAttribute("admin","登入成功");
				return "manage/index";
			}
		}
		return "manage/login";
	...
	public String manaUser(Integer pageNum,Model model){
		if(pageNum!=null){
			PageHelper.startPage(pageNum,com.laver.mbook.util.Constant.MU_PAGE_SIZE);
		}else{
			PageHelper.startPage(1, com.laver.mbook.util.Constant.MU_PAGE_SIZE);
		}
